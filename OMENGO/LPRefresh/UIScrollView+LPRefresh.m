...
//    [self centerSub:self.bounds.size.width];
//}
...
- (void)addRefreshWithBlock:(void (^)())block
{
    ...
    [RACObserve(self, contentOffset) subscribeNext:^(id x) {
        @strongify(self);
        //下拉进度
        //设置用户是否在交互,这里主要是设置用户是否在拖动屏动
        UIGestureRecognizerState gestureState = self.panGestureRecognizer.state;
        self.indicator.isDragging = gestureState ==  UIGestureRecognizerStateChanged;
